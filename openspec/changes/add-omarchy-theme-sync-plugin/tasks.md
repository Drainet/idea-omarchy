...
- [ ] 1.1 Create the Kotlin/Gradle IntelliJ Platform plugin project, select and document a supported IntelliJ IDEA Community baseline, and verify `./gradlew verifyPluginProjectConfiguration` succeeds.
- [ ] 1.2 Configure plugin identity, Linux/Omarchy compatibility metadata, application services, settings configurable, and action registrations; verify the sandbox IDE loads the plugin without descriptor errors.
...
- [ ] 2.1 Implement immutable palette, source-path, parse-result, and diagnostic models; verify unit tests cover dark and light real `colors.toml` fixtures.
- [ ] 2.2 Implement TOML parsing and semantic validation for mode, required palette entries, RGB/RGBA syntax, regular-file checks, and alternate paths; verify tests reject missing, malformed, unreadable, and unsupported input without returning a palette.
- [ ] 2.3 Implement normalized palette hashing and source reconciliation so unchanged content does not trigger an update; verify unit tests produce one update for a changed palette and none for equivalent formatting or unrelated events.
- [ ] 2.4 Implement a disposable, debounced filesystem monitor for the default Omarchy state parent and configured alternate paths, including bounded retry and watch re-registration; verify integration tests replace a temporary `theme` directory and observe exactly one valid palette notification.
...
- [ ] 3.1 Implement color conversion, luminance/contrast checks, and semantic fallback rules; verify unit tests cover valid RGB/RGBA conversion and contrast-safe light and dark fallback choices.
...
- [ ] 6.2 Run IntelliJ Plugin Verifier against the documented baseline and current supported IntelliJ IDEA release; verify the verification report contains no compatibility errors.
...
- [ ] 6.4 Write installation, permissions/privacy, supported-platform, troubleshooting, and rollback documentation; verify `./gradlew buildPlugin` produces an installable ZIP referenced by the README.
